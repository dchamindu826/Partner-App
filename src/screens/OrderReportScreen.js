...
const OrderReportScreen = () => {
    ...
    const handleGenerateReport = async () => {
        ...
        try {
            ...
            setReportData(data);
            const total = data.reduce((sum, order) => sum + ((order.foodTotal || 0) * 0.95), 0);
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
                <View style={styles.card}>
                    ...
                    <View style={styles.dateRow}>
                        <TouchableOpacity style={styles.dateBox} onPress={() => setShowPicker('start')}>
                            <View style={styles.iconBox}><Ionicons name="calendar-outline" size={20} color={COLORS.primaryYellow} /></View>
                            <View>
                ...
                <View style={styles.summaryCard}>
                    <Text style={styles.summaryLabel}>Total Net Earning (95%)</Text>
                        ...
                        {reportData.map((order, index) => (
                            <View key={order._id} style={[styles.tableRow, index % 2 === 0 && styles.rowAlt]}>
                                ...
                                <Text style={[styles.td, {flex: 2, textAlign: 'right', color: COLORS.success, fontWeight: 'bold'}]}>
                                    {(order.foodTotal * 0.95).toFixed(2)}
                                </Text>
                            </View>
                        ))}
                    </View>
                ) : (
                    <View style={styles.emptyState}><Text style={styles.emptyText}>No orders found.</Text></View>
                )}
...
const styles = StyleSheet.create({
    ...
    headerContainer: { flexDirection: 'row', alignItems: 'center', padding: 20, backgroundColor: COLORS.white, borderBottomWidth: 1, borderBottomColor: COLORS.border },
    ...
    card: { backgroundColor: COLORS.white, borderRadius: 12, padding: 15, marginBottom: 20, elevation: 2 },
    presetRow: { flexDirection: 'row', marginBottom: 15 },
    presetChip: { paddingVertical: 6, paddingHorizontal: 15, borderRadius: 20, backgroundColor: COLORS.lightBackground, marginRight: 10, borderWidth: 1, borderColor: COLORS.border },
    ...
    dateBox: { flex: 1, flexDirection: 'row', alignItems: 'center', padding: 10, backgroundColor: COLORS.lightBackground, borderRadius: 8 },
    iconBox: { width: 36, height: 36, borderRadius: 18, backgroundColor: COLORS.white, alignItems: 'center', justifyContent: 'center', marginRight: 10 },
    ...
    summaryCard: { backgroundColor: '#333', borderRadius: 12, padding: 20, marginBottom: 25, alignItems: 'center' },
    ...
    tableRow: { flexDirection: 'row', paddingVertical: 15, paddingHorizontal: 12, borderBottomWidth: 1, borderBottomColor: COLORS.border },
